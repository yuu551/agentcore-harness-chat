...
| 費用目安 | WAF の固定費（約 $6〜7/月） | WAF の固定費に加えて Amplify Firewall の利用料が別途発生 |
| 向いている場合 | アクセス制御として IP を縛りたい（推奨の既定） | ツールの存在自体を外部から隠したい |

- 迷ったら **A（テンプレートの WAF）** を推奨します。実害のある操作（認証・API）を縛るのはこちらで、設定がコードとして残ります
- 両方を併用すると「完全に閉じた」状態にできますが、WAF のスコープ（REGIONAL / CLOUDFRONT）が異なるため **Web ACL は必ず 2 つ**になり、許可リストの管理も二重になります。コンプライアンス要件がある場合のみ検討してください
...
> Amplify Firewall は CloudFront スコープの WAF が別途作成されるため、テンプレートの WAF とは統合できません。WAF の費用に加えて Amplify Firewall の利用料も別途発生します（金額はコンソールの見積もり表示を参照）。冒頭の[選択ガイド](#どちらを使うか選択ガイド)のとおり、明確な要件がなければテンプレート側の WAF だけで運用を始めるのがおすすめです。
